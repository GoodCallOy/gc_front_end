<template>
  ...
  export default {
    ...
    methods: {
      ...
      async submitForm() {
        try {
          const firstElement = this.agent.goal_date?.[0] 
            ? new Date(this.agent.goal_date[0]).toISOString() 
            : null;

          const lastElement = this.agent.goal_date?.[this.agent.goal_date.length - 1] 
            ? new Date(this.agent.goal_date[this.agent.goal_date.length - 1]).toISOString() 
            : null;
          ...
          console.log('Agent goals added: ', response);
          this.message = 'goal added successfully!';
